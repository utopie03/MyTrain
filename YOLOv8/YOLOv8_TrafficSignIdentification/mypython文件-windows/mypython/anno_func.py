...
def load_mask(annos, datadir, imgid, imgdata):
    ...
    for obj in img['objects']:
        ...
        if 'ellipse' in obj:
            ...
            cv2.rectangle(mask_ellipse, (int(box['xmin']), int(box['ymin'])), (int(box['xmax']), int(box['ymax'])), 1,
                          -1)
...
def get_refine_rects(annos, raw_rects, minscore=20):
    ...
    for imgid in list(raw_rects.keys()):
        v = raw_rects[imgid]
    ...
    results = {}
    for imgid, v in list(refine_rects.items()):
        objs = []
        for obj in v:
            mobj = {"bbox": dict(list(zip(["xmin", "ymin", "xmax", "ymax"], obj[1]))),
...
def eval_annos(annos_gd, annos_rt, iou=0.75, imgids=None, check_type=True, types=None, minscore=40, minboxsize=0,
               maxboxsize=400, match_same=True):
    ac_n, ac_c = 0, 0
    rc_n, rc_c = 0, 0
    if imgids == None:
        imgids = list(annos_rt['imgs'].keys())
    if types != None:
    ...
    report = "iou:%s, size:[%s,%s), types:%s, accuracy:%s, recall:%s" % (
        iou, minboxsize, maxboxsize, styps, 1 if ac_n == 0 else ac_c * 1.0 / ac_n,
        1 if rc_n == 0 else rc_c * 1.0 / rc_n)
    summury = {
        "iou": iou,
