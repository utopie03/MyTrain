...
#### 1. 专业综合——MNIST手写体识别
> 本实践旨在通过基于深度学习框架PyTorch的手写数字识别模型，实现对`MNIST`手写数字数据集的准确识别。
...
#### 2. 生产实习——YOLOv8交通标识识别
> 本次实验将使用YOLOv8训练TT100K中国交通标志数据集，完成一个多目标检测实战项目，可实时检测图像、视频、摄像头和流媒体(http/rtsp)中的交通标志，并提供`PySide6`开发的可视化演示界面。
